...
function Pokemon.summon(player, item)
	...
	-- Walking speed is **not** set here. It belongs to the species and lives on
	-- the MonsterType, because a Scyther is meant to outrun a Slowpoke and
	-- matching the trainer would erase exactly that difference. A pokemon too
	-- slow to keep up is supposed to fall behind -- that is what the teleport
	-- in `PokemonFollowTrainer` is for.
